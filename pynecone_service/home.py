...
from. base_state import User, Question, MbtiResult
...
class State(pc.State):
    ...
    @pc.var
    def alert_id(self):
        if re.search(r'^[A-Za-z0-9]{4,20}$', self.userid):
            return False
        else:
            return True
    ...
    def load_user(self, target_userid=None):
        ...
        self.target_userid = re.sub(r"[^A-Za-z0-9]", "", str(self.target_userid))
        ...
        with pc.session() as session:
            ...
            if exist_user:
                ...
                self.target_results = session.query(MbtiResult).where(MbtiResult.target_userid == self.target_userid, MbtiResult.userid != self.target_userid).order_by(MbtiResult.score.desc()).all()
    ...
    def get_mbti(self):
        ...
        with pc.session() as session:
            target_answer = session.query(MbtiResult).where(MbtiResult.userid == users["target_userid"], MbtiResult.target_userid == users["target_userid"]).first()
            ...
            if session.query(MbtiResult).where(MbtiResult.userid == users["userid"], MbtiResult.target_userid == users["target_userid"]).all():
                session.query(MbtiResult).where(MbtiResult.userid == users["userid"], MbtiResult.target_userid == users["target_userid"]).update({"userid": users["userid"], "target_userid": users["target_userid"], "username": users["username"], "target_username": users["target_username"], "mbti": info["mbti"], "score": round(info["score"], 2), "question_result": str(info["question_result"])})
            else:
    ...
    @pc.var
    def get_target_userid(self):
        return self.target_data["user"]["target_userid"]
...
def login():
    return pc.center(
        pc.vstack(
            pc.input(on_blur=State.set_userid, placeholder="아이디 입력", width="100%"),
            pc.input(type_="password", on_blur=State.set_password, placeholder="비밀번호 입력", width="100%"),
                    ...
                    pc.vstack(
                        ...
                        pc.cond(
                            ...
                            pc.alert(
                                pc.alert_icon(),
                                pc.alert_title(
                                    "아이디는 영어 대소문자, 숫자만을 활용하여 4자 이상 20자 이하로 입력해주세요"
                                ),
                                status="error"
                            )
                        ),
                ...
                pc.center(
                    pc.vstack(
                        pc.cond(
                                    ...
                                    pc.vstack(
                                        pc.cond(
                                            ...
                                            pc.vstack(
                                                ...
                                                pc.text(State.get_target_mbti + "입니다, 점수는 " + State.get_target_score+"점 입니다"),
                            ...
                            pc.heading("존재하지 않는 아이디입니다"),
                        ),
                        pc.center(
                            pc.input(on_blur=State.set_target_userid, placeholder="친구 아이디 입력", width="100%"),
                        ...
                        pc.cond(
                            ...
                            pc.vstack(
                                pc.text("내 페이지 공유하기"),
                                pc.text("localhost:3000/"+State.userid)
                            )
                        )
                    ),
                    shadow="lg",
                ...
                pc.vstack(
                    pc.cond(
                        ...
                        pc.vstack(
                            pc.text(State.username+"님이 생각하시는 "+State.get_target_username+"님의 MBTI는 "),
                            pc.text(State.get_target_mbti+"입니다, 점수는 "+State.get_target_score+"점 입니다"),
                            pc.button(State.get_target_username+"님 페이지로 이동하기", on_click=lambda: State.load_user(State.get_target_userid), width="100%"),
                        ),
                        login()
                    ),
                ),
