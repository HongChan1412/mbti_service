...
import {Avatar, Box, Button, Center, Divider, HStack, Heading, Link, Menu, MenuButton, MenuDivider, MenuItem, MenuList, Progress, Text, VStack, useColorMode} from "@chakra-ui/react"
...
{state.question_data_state ? <Center sx={{"shadow": "lg", "padding": "1em", "borderRadius": "lg", "background": "white"}}><VStack><Heading>{state.get_ask}</Heading>
<HStack><Button onClick={() => Event([E("state.change_answer", {question_answer:0})])}
sx={{"color": state.get_color1}}>{state.get_answer_1}</Button>
<Center sx={{"height": "4em"}}><Divider orientation="vertical"
sx={{"borderColor": "black"}}/></Center>
<Button onClick={() => Event([E("state.change_answer", {question_answer:1})])}
sx={{"color": state.get_color2}}>{state.get_answer_2}</Button></HStack>
<HStack><Button onClick={() => Event([E("state.prev_question", {})])}>{`<`}</Button>
<Button onClick={() => Event([E("state.next_question", {})])}>{`>`}</Button></HStack>
<VStack spacing="1em"
sx={{"minWidth": ["10em", "20em"]}}><Progress value={state.question_progress}
sx={{"width": "100%"}}/></VStack></VStack></Center> : <Center><VStack>{state.logged_in ? <Button onClick={() => Event([E("state.load_question", {})])}>{`테스트 진행하기`}</Button> : <Center><NextLink passHref={true}
href="/"><Link sx={{"width": "100%"}}><Button sx={{"width": "100%"}}>{`로그인`}</Button></Link></NextLink></Center>}</VStack></Center>}</VStack>
