...
import {Avatar, Box, Button, Center, Divider, HStack, Heading, Input, Link, Menu, MenuButton, MenuDivider, MenuItem, MenuList, Progress, Text, VStack, useColorMode} from "@chakra-ui/react"
...
{state.question_data_state ? <Center sx={{"shadow": "lg", "padding": "1em", "borderRadius": "lg", "background": "white"}}><VStack><Heading>{("\uc9c8\ubb38" + state.question_idx)}</Heading>
<Text>{state.get_ask}</Text>
<HStack><Button onClick={() => Event([E("state.change_answer", {question_answer:0})])}
sx={{"bg": state.get_color1}}>{state.get_answer_1}</Button>
...
sx={{"bg": state.get_color2}}>{state.get_answer_2}</Button></HStack>
...
sx={{"width": "100%"}}/></VStack></VStack></Center> : <Center><VStack>{state.logged_in ? <Button onClick={() => Event([E("state.load_question", {})])}>{`테스트 진행하기`}</Button> : <Center sx={{"shadow": "lg", "padding": "1em", "borderRadius": "lg", "background": "white"}}><VStack><Input type="text"
placeholder="Userid"
