...
import {Avatar, Box, Button, Center, HStack, Heading, Input, Link, Menu, MenuButton, MenuDivider, MenuItem, MenuList, Table, TableContainer, Tbody, Td, Text, Th, Thead, Tr, VStack, useColorMode} from "@chakra-ui/react"
...
<Fragment>{state.logged_in ? <Fragment><Center sx={{"shadow": "lg", "padding": "1em", "borderRadius": "lg", "background": "white"}}><VStack><Fragment>{state.exist_user ? <Fragment><VStack><Heading>{(state.get_target_username + "\ub2d8\uc758 \ud398\uc774\uc9c0")}</Heading>
<Fragment>{state.exist_result ? <Fragment><VStack><Fragment>{state.exist_answer ? <Fragment><VStack><Text>{(((state.username + "\ub2d8\uc774 \uc0dd\uac01\ud558\uc2dc\ub294 ") + state.get_target_username) + "\ub2d8\uc758 MBTI\ub294 ")}</Text>
<Text>{(((state.get_target_mbti + "\uc785\ub2c8\ub2e4, \uc810\uc218\ub294 ") + state.get_target_score) + "\uc785\ub2c8\ub2e4")}</Text>
<Button onClick={() => Event([E("state.load_question", {})])}
sx={{"width": "100%"}}>{(state.get_target_username + "\ub2d8\uc758 MBTI \ud14c\uc2a4\ud2b8 \ub2e4\uc2dc\ud558\uae30")}</Button></VStack></Fragment> : <Fragment><Button onClick={() => Event([E("state.load_question", {})])}
sx={{"width": "100%"}}>{(state.get_target_username + "\ub2d8\uc758 MBTI \ud14c\uc2a4\ud2b8\ud558\uae30")}</Button></Fragment>}</Fragment>
<TableContainer><Table><Thead><Tr><Th>{`username`}</Th>
<Th>{`mbtiresult`}</Th>
<Th>{`score`}</Th></Tr></Thead>
<Tbody>{state.target_results.map((kbixhbhq, i) => <Tr key={i}><Td>{kbixhbhq.username}</Td>
<Td>{kbixhbhq.mbti}</Td>
<Td>{kbixhbhq.score}</Td></Tr>)}</Tbody></Table></TableContainer></VStack></Fragment> : <Fragment><VStack><Fragment>{state.judge_mypage ? <Fragment><Button onClick={() => Event([E("state.load_question", {})])}
sx={{"width": "100%"}}>{(state.get_target_username + "\ub2d8\uc758 MBTI \ud14c\uc2a4\ud2b8\ud558\uae30")}</Button></Fragment> : <Fragment><Text>{(state.get_target_username + "\ub2d8\uc774 MBTI \ud14c\uc2a4\ud2b8\ub97c \uc9c4\ud589\ud558\uc9c0 \uc54a\uc73c\uc168\uc2b5\ub2c8\ub2e4")}</Text></Fragment>}</Fragment></VStack></Fragment>}</Fragment></VStack></Fragment> : <Fragment><Heading>{`존재하지 않는 userid`}</Heading></Fragment>}</Fragment>
<Center><Input onBlur={(_e) => Event([E("state.set_target_userid", {target_userid:_e.target.value})])}
placeholder="Userid"
sx={{"width": "100%"}}
type="text"/>
<Button onClick={() => Event([E("state.load_user", {target_userid:state.target_userid})])}
sx={{"width": "100%"}}>{`친구 페이지로 이동하기`}</Button></Center>
<Button onClick={() => Event([E("state.load_user", {target_userid:state.userid})])}
sx={{"width": "100%"}}>{`마이페이지로 이동하기`}</Button></VStack></Center></Fragment> : <Fragment><Center sx={{"shadow": "lg", "padding": "1em", "borderRadius": "lg", "background": "white"}}><VStack><Input onBlur={(_e) => Event([E("state.set_userid", {userid:_e.target.value})])}
placeholder="Userid"
sx={{"width": "100%"}}
type="text"/>
