...
import {Alert, AlertIcon, AlertTitle, Avatar, Box, Button, Center, HStack, Heading, Input, Link, Menu, MenuButton, MenuDivider, MenuItem, MenuList, Text, VStack, useColorMode} from "@chakra-ui/react"
...
passHref={true}><Link sx={{"width": "100%"}}><Button sx={{"width": "100%"}}>{`마이페이지`}</Button></Link></NextLink></Center></Fragment> : <Fragment><Center sx={{"shadow": "lg", "padding": "1em", "borderRadius": "lg", "background": "white"}}><VStack><Heading sx={{"fontSize": "1.5em"}}>{`MBTI 테스트 회원가입`}</Heading>
<Input onBlur={(_e) => Event([E("state.set_username", {username:_e.target.value})])}
placeholder="Username"
sx={{"width": "100%"}}
type="text"/>
<Fragment>{state.alert_username_set ? <Fragment><Alert status="error"><AlertIcon/>
<AlertTitle>{`이름은 한글, 영어 대소문자, 숫자와 '_'기호만을 활용하여 2자 이상 20자 이하로 입력해주세요`}</AlertTitle></Alert></Fragment> : <Fragment/>}</Fragment>
<Input onBlur={(_e) => Event([E("state.set_userid", {userid:_e.target.value})])}
placeholder="Userid"
sx={{"width": "100%"}}
type="text"/>
<Fragment>{state.alert_id ? <Fragment><Alert status="error"><AlertIcon/>
<AlertTitle>{`아이디는 영어 대소문자, 숫자와 '_'기호만을 활용하여 4자 이상 20자 이하로 입력해주세요`}</AlertTitle></Alert></Fragment> : <Fragment/>}</Fragment>
<Input onBlur={(_e) => Event([E("state.set_password", {password:_e.target.value})])}
placeholder="Password"
sx={{"width": "100%"}}
type="password"/>
<Fragment>{state.alert_password ? <Fragment><Alert status="error"><AlertIcon/>
<AlertTitle>{`비밀번호는 영문 대소문자, 숫자, 특수문자(!@#$%^&*?)를 사용하되 8자 이상 20자 이한으로 입력해주세요`}</AlertTitle></Alert></Fragment> : <Fragment/>}</Fragment>
<Input onBlur={(_e) => Event([E("state.set_confirm_password", {confirm_password:_e.target.value})])}
placeholder="Password Confirm"
sx={{"width": "100%"}}
type="password"/>
<Fragment>{state.alert_confirm_password ? <Fragment><Alert status="error"><AlertIcon/>
<AlertTitle>{`비밀번호를 확인해주세요`}</AlertTitle></Alert></Fragment> : <Fragment/>}</Fragment>
<Button isDisabled={state.judge_signup}
onClick={() => Event([E("state.signup", {})])}
sx={{"width": "100%"}}>{`회원가입`}</Button></VStack></Center></Fragment>}</Fragment></VStack>
